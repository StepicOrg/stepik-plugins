...
class TableQuizTest(unittest.TestCase):
    def setUp(self):
        ...
        self.quizzes = [TableQuiz(TableQuiz.Source(source)) for source in sources]


class ChoiceQuizInitTest(TableQuizTest):
    def test_invalid_sources(self):
        ...
        for bad_source in [dict(self.default_source, **d) for d in diff]:
            with self.assertRaises(FormatError):
                quiz = TableQuiz(TableQuiz.Source(bad_source))
    ...
    def test_valid_sources(self):
        ...
        for good_source in [dict(self.default_source, **d) for d in diff]:
            TableQuiz(TableQuiz.Source(good_source))
...
class ChoiceQuizCleanReplyTest(TableQuizTest):
    def setUp(self):
        super().setUp()
        self.quiz = TableQuiz(TableQuiz.Source(self.default_source))
        self.dataset = TableQuiz.Dataset({'is_multiple_choice': self.quiz.is_multiple_choice,
                                           'options': ['Foo', 'bar']})

    def test_invalid_reply(self):
        with self.assertRaises(FormatError):
            self.quiz.clean_reply(
                reply=TableQuiz.Reply({'choices': [True]}), dataset=self.dataset)

        with self.assertRaises(FormatError):
            self.quiz.clean_reply(
                reply=TableQuiz.Reply({'choices': [True, True]}), dataset=self.dataset)

    def test_options_extracted_from_reply(self):
        cleaned_reply = self.quiz.clean_reply(
            reply=TableQuiz.Reply({'choices': [True, False]}), dataset=self.dataset)

        self.assertEqual(cleaned_reply, [True, False])


class ChoiceQuizCheckTest(TableQuizTest):
    ...
    def test_multiple_choice(self):
        quiz = TableQuiz(TableQuiz.Source(dict(self.default_source, is_multiple_choice=True)))
    ...
    def test_anyone_correct(self):
        quiz = TableQuiz(TableQuiz.Source(dict(self.default_source, is_always_correct=True)))
        self.assertIs(quiz.check([True, False, False], [True, False, False]), True)
        self.assertIs(quiz.check([False, True, False], [True, False, False]), True)


class ChoiceQuizGenerateTest(TableQuizTest):
    def test_generate(self):
        ...
        for quiz_instance in self.quizzes:
            ...
            dataset_instance = TableQuiz.Dataset(dataset_dict)
            for pred in predicates:
                pred(quiz_instance, dataset_instance, clue_instance)


class ChoiceQuizSolveTest(TableQuizTest):
    def test_all(self):
        ...
        for quiz in self.quizzes:
            ...
            dataset = TableQuiz.Dataset(dataset)
            reply = TableQuiz.Reply(solve(quiz, dataset))
